...
**Project status:** Operational research software, version `0.1.0`. The repository demonstrates product development, documentation, reproducibility, validation, public sharing, and a live evidence dashboard. Formal registration or deployment within the CGIAR/ILRI Climate Data Hub should be documented separately before the product is reported as fully integrated.
...
[![AI Climate Data Agent data inventory](docs/images/dashboard-inventory.svg)](https://yonsci.github.io/climate-agent/)
...
[![AI Climate Data Agent run history](docs/images/dashboard-run-history.svg)](https://yonsci.github.io/climate-agent/)
...
The dashboard is generated by [`scripts/generate_report.py`](scripts/generate_report.py) and published through [`.github/workflows/pages.yml`](.github/workflows/pages.yml). It is refreshed when the configured GitHub Pages workflow runs after relevant evidence files are updated.
...
| **Develop and share the product** | Shares source code, configuration, examples, workflows, issue templates, contribution guidance, Wiki content, and a public evidence dashboard. |
...
| Review all processed datasets without reading JSON files. | Opens the [live dashboard](https://yonsci.github.io/climate-agent/) to inspect inventory, run history, validation, QC, and diagnostics. |
...
| **Products** | NetCDF, JSON manifests, QA JSON, PNG diagnostics, logs, delivery inventories, HTML reports, and a public evidence dashboard |
...
### 5. Review the published evidence
...
Each formal delivery should record the Git commit SHA, version tag, configuration, source versions and access dates, run manifest, delivery manifest, dashboard URL, and associated Hub record.
...
- published or archived provenance, QA, dashboard, and delivery evidence;
...
- [ ] Dashboard URL and representative screenshot included in reporting evidence
...
> Mersha, Y. *Climate Data Harmonization Agent: automated acquisition, harmonization, validation, dashboard reporting, and delivery of climate datasets*. ILRI Climate Services. Version 0.1.0. GitHub repository: `YonSci/climate-agent`. Dashboard: `https://yonsci.github.io/climate-agent/`.
